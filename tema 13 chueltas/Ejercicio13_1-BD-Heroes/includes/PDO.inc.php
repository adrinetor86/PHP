 ...
 * Si la conexión es correcta, devuelve un objeto PDO,
 * si no lo es devuelve un null.
 *
 * @return PDO|null Resultado de la función. PDO si la conexión es
 *                  exitosa. null si no lo es.
 */
function generaConexion(): PDO|null {
    $strGestorBD = 'mysql';
    $strHost = 'localhost';
    $strPuerto = '3310';
    $strDbname = 'juego';
...
function select(): void {
    $strTabla = 'HEROES';
    $strQuery = 'SELECT * FROM ' . $strTabla;

    try {
        $conexion = generaConexion();
        if ($conexion) {
            $sentenciaSQL = $conexion->prepare($strQuery);
            $sentenciaSQL->setFetchMode(PDO::FETCH_ASSOC);
            $sentenciaSQL->execute();

            if ($sentenciaSQL->rowCount() > 0) {
                $arrCamposTabla = obtenCamposTabla($strTabla, $conexion);
                generaTablaHTML($arrCamposTabla, $sentenciaSQL);
            } else {
...
function selectHeroe(string $id): void {
    $strTabla = 'HEROES';
    $strQuery = 'SELECT * FROM ' . $strTabla . ' WHERE ID = :id';
...
function update(array $arrDatos): void {
    ...
    try {
        ...
        if ($conexion) {
            $sentenciaSQL = $conexion->prepare($strQuery);
            $sentenciaSQL->bindParam(':nombre', $arrDatos['nombre']);
            $sentenciaSQL->bindParam(':rol', $arrDatos['rol']);
            $sentenciaSQL->bindParam(':dificultad', $arrDatos['dificultad']);
            $sentenciaSQL->bindParam(':descripcion', $arrDatos['descripcion']);
            $sentenciaSQL->bindParam(':id', $arrDatos['id']);
            $sentenciaSQL->execute();
        }
    } catch (PDOException $e) {
        echo 'Error al actualizar datos en la tabla ' . $strTabla . '.';
    } finally {
        if (isset($conexion)) {
            unset($conexion);
        }
...
function delete(string $id): void {
    $strTabla = 'HEROES';
    $strQuery = 'DELETE FROM ' . $strTabla . ' WHERE ID = :id';
    ...
    try {
        ...
        echo 'Error al borrar datos en la tabla ' . $strTabla . '.';
    } finally {
        if (isset($conexion)) {
            unset($conexion);
        }
 ...
 *
 * @param string $tabla Nombre de la tabla de la cual se quieren conocer los campos.
 * @param PDO $conexion Objeto PDO que contiene la conexión a la BD.
 * @return array - Devuelve un array con el nombre de los campos de la tabla.
 */
function obtenCamposTabla(string $tabla, PDO $conexion): array {
    $arrCamposReturn = array();
    $strQuery = 'DESCRIBE ' . $tabla;
    ...
    try {
        ...
        while ($campo = $sentenciaSQL->fetch()) {
            $arrCamposReturn[] = $campo['Field'];
        }
    } catch (PDOException $e) {
        echo 'Error durante el acceso a los campos de la tabla.';
    } finally {
        $sentenciaSQL->closeCursor();
    }
 ...
 *
 * @param PDOStatement $sentenciaSQL PDOStatement que contiene la consulta que ha sido realizada sobre la tabla.
 * @return string Devuelve un string que contiene las etiquetas HTML que conforman el cuerpo de la tabla.
 */
function generaTbody(PDOStatement $sentenciaSQL): string {
    $strReturn = '<tbody>';

    while ($heroe = $sentenciaSQL->fetch()) {
        $strReturn .= '<tr>';
        $arrAtributos = array_values($heroe);

        foreach ($arrAtributos as $atributo) {
            $strReturn .= '<td>' . $atributo . '</td>';
        }
        $strReturn .= '<td><form method="post">' .
            generaBoton($heroe['ID'], './php/editar_heroe.php', 'Editar') .
            '</form></td><td><form method="post">' .
            generaBoton($heroe['ID'], './php/borrar_heroe.php', 'Borrar') .
            '</form></td>';
    }
